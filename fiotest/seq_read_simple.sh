...
set -euo pipefail
# ================================================
# シーケンシャルリード試験（128k, QD32, 1job）
# ・目的：シーケンシャル読出しのスループット測定
# ・注意：プレコンは破壊的（書込み）です。必要に応じて確認のうえ実行してください。
# ================================================
TEST_NAME="seqread"
# ========= 共通設定 =========
...
echo "=== SeqRead を ${RUNTIME}s 実行します…（DEV=$DEV）"
fio --name=seqread     --filename="$DEV" --ioengine="$IOENGINE" --direct=1     --time_based=1 --runtime="$RUNTIME"     --rw=read --bs=128k --iodepth=32 --numjobs=1     --group_reporting=1     --output="$JSON_OUT" --output-format=json
RC=$?
if [[ $RC -ne 0 ]]; then
