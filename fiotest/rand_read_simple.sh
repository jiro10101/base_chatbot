...
set -euo pipefail
# ================================================
# ランダムリード試験（4k, QD64, 4jobs）
# ・目的：4Kランダム読出しのIOPS/レイテンシ測定
# ================================================
TEST_NAME="randread"
# ========= 共通設定 =========
...
echo "=== RandRead を ${RUNTIME}s 実行します…（DEV=$DEV）"
fio --name=randread     --filename="$DEV" --ioengine="$IOENGINE" --direct=1     --time_based=1 --runtime="$RUNTIME"     --rw=randread --bs=4k --iodepth=64 --numjobs=4     --group_reporting=1     --output="$JSON_OUT" --output-format=json
RC=$?
if [[ $RC -ne 0 ]]; then
