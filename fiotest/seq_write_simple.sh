...
set -euo pipefail
# ================================================
# シーケンシャルライト試験（128k, QD32, 1job）
# ・目的：シーケンシャル書込みのスループット測定
...
echo "=== SeqWrite を ${RUNTIME}s 実行します…（DEV=$DEV）"
fio --name=seqwrite     --filename="$DEV" --ioengine="$IOENGINE" --direct=1     --time_based=1 --runtime="$RUNTIME"     --rw=write --bs=128k --iodepth=32 --numjobs=1     --group_reporting=1     --output="$JSON_OUT" --output-format=json
RC=$?
if [[ $RC -ne 0 ]]; then
