...
`fio` の使い方を知らないユーザーでも、チャットボットと会話しながら試験を実施・管理できるようにする。

## 成功基準

- `fio` の使い方を知らなくても試験を実行できる
...
1. **コマンド自動生成**（成功基準①）: fio の知識がなくても、会話形式でパラメータを収集し、試験コマンド（`Testtoolsqript.sh`）を自動生成する
...
3. 正しい形式なら確認メッセージを表示、不正なら LLM に問い合わせてコマンド候補を生成（**成功基準①**: fio を知らなくても試験を実行できる）
4. 有効なコマンドがある場合、`exec_result` が未実行（None）または失敗のときのみサイドバーに「Screen 実行」ボタンを表示（成功後は非表示）
